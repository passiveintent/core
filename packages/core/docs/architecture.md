...
1. Records state transitions (page routes, UI milestones, custom events) into a sparse **Markov graph** stored in the active `StorageAdapter` — browser apps typically use `localStorage` via `createBrowserIntent()` or `BrowserStorageAdapter`.
...
14. Detects **exit intent** — when the user's pointer leaves the viewport toward the browser chrome, a `exit_intent` event fires **only if** the Markov graph predicts a likely continuation path (P ≥ 0.4). Spammy overlays are structurally impossible: the event is suppressed when the graph has no high-confidence next state.
...
| **Dirty-flag persistence**    | `StorageAdapter` writes only happen when state actually changed, eliminating jank on high-frequency routes. When `BrowserStorageAdapter` is in use, that means fewer `localStorage` writes.                                                                                                                                                                                                             |
...
| **Smart exit-intent**         | When the user's pointer exits the viewport from above (toward the browser chrome) and the Markov graph has at least one continuation candidate with probability ≥ 0.4, an `exit_intent` event fires with the predicted next state. If no candidate meets the threshold the event is suppressed entirely, making data-free overlays structurally impossible. Requires `LifecycleAdapter.onExitIntent()`. |
...
When `BrowserStorageAdapter` is active (directly or via `createBrowserIntent()`), `persist()` writes to `localStorage`. Browsers allow 5–10 MB per origin. The library's binary format keeps the payload to ~1.4 kB for 100 states, but extremely large graphs (thousands of states) can hit quota limits. Subscribe to `onError` to handle `QuotaExceededError` gracefully — the callback will receive `{ code: 'QUOTA_EXCEEDED', ... }` and `getTelemetry().engineHealth` will read `'quota_exceeded'`.
...
interface IntentManagerConfig {
  ...
  storage?: StorageAdapter; // default: MemoryStorageAdapter (volatile Map-backed storage)
...
| **LifecycleCoordinator**                             | Owns pause/resume handling — adjusts `previousStateEnteredAt` on tab hide/show; emits `session_stale` when hidden duration exceeds `MAX_PLAUSIBLE_DWELL_MS`; emits `attention_return` for comparison-shopper detection (≥ 15 s hide); tracks user interactions and emits `user_idle` / `user_resumed` for idle-state detection; subscribes to `adapter.onExitIntent()` when present and invokes the `IntentManager`-supplied Markov-validation callback that conditionally emits `exit_intent` | SSR-safe; accepts `null` adapter to disable entirely; conditionally owns or borrows the `LifecycleAdapter` based on whether one was injected via config  |
...
### Smart Exit-Intent Detection (exit_intent)

When a user moves the pointer toward the browser address bar or title bar they are typically about to navigate away or close the tab — a narrow, high-value window for a last-chance intervention. Existing solutions fire an overlay on _every_ `mouseleave`, even when the user has barely engaged with the page. PassiveIntent takes a different approach: the `exit_intent` event fires **only when the Markov graph has already learned a high-confidence continuation path** (probability ≥ 0.4) from the current state. If the graph is empty or the user is on an isolated page, the event is silently suppressed and the user is never interrupted.

**How it works:**

1. `BrowserLifecycleAdapter.onExitIntent(callback)` attaches a `mouseleave` listener on `document.documentElement`.
2. The handler fires `callback()` only when `event.clientY <= 0` (pointer exited from the top edge — i.e., toward the browser chrome). Left/right/bottom exits are ignored.
3. `LifecycleCoordinator` subscribes to this callback at construction time and delegates to an `onExitIntent()` hook supplied by `IntentManager`.
4. `IntentManager`'s hook performs the Markov check:
   - If `previousState === null`: return, do nothing.
   - `candidates = graph.getLikelyNextStates(previousState, 0.4)` — threshold of 0.4 filters noise.
   - If `candidates.length === 0`: return, event suppressed.
   - Emit `exit_intent` with `{ state: previousState, likelyNext: candidates[0].state }`.
...
export interface ExitIntentPayload {
  ...
  state: string;
  /**
   * The most probable next state according to the Markov graph.
   * Always a non-null string when the event fires — a null result suppresses
...
**Usage — Checkout Retention Offer:**
...
intent.on('exit_intent', ({ state, likelyNext }) => {
  if (state === '/checkout/payment') {
    UI.showModal({
      ...
      message: `You were about to head to ${likelyNext}. Need help?`,
...
**`BrowserLifecycleAdapter.onExitIntent()`:**

The default browser implementation attaches a single `mouseleave` listener on `document.documentElement`. The listener is lazy — it is only attached when the first subscriber calls `onExitIntent()` — and it is torn down on unsubscription of the last callback or on `destroy()`. The `clientY <= 0` guard ensures that normal in-page mouse movement (navigating to a sidebar, scrolling via scrollbar) never triggers the callback.

In SSR or Node.js environments, `document` is absent so `onExitIntent()` is never called and no listener is registered.

**Backward compatibility:**

Custom `LifecycleAdapter` implementations that do not implement `onExitIntent` continue to work unchanged. The coordinator checks `typeof adapter.onExitIntent === 'function'` before attempting to subscribe. Exit-intent detection is simply disabled for those adapters.
...
| **Browser-local when enabled**      | Persistence routes exclusively through the `StorageAdapter` interface. Plain `new IntentManager()` uses the in-memory `MemoryStorageAdapter`; `createBrowserIntent()` and `storage: new BrowserStorageAdapter()` write to `localStorage`. Inspect the default persisted state at `localStorage.getItem('passiveintent:passive-intent')`. |
...
| **User can clear state**            | When browser persistence is enabled, `localStorage.removeItem('passiveintent:passive-intent')` wipes all learned state. With the default in-memory adapter, a page reload clears it automatically.                                                                                                                                       |
...
The net result: the clean **"no cookie banner, no consent prompt, and no opt-out mechanism"** headline applies to the default zero-egress, in-memory deployment. If you enable persistent browser storage or export graph data to your own infrastructure, assess ePrivacy / GDPR obligations with your own counsel. No DPA with PassiveIntent is required because PassiveIntent does not receive the data.
